  ...
  return (
    <Card className={classes['recipe-card-container']}>
      ...
      <CardActions disableSpacing>
        ...
        <a className={classes.source} href={food.recipe.url}>View Source</a>
        <ExpandMore
          expand={expanded}
      ...
      <Collapse in={expanded} timeout='auto' unmountOnExit>
        <CardContent className={classes['recipe-content-container']}>
          <a className={classes.edamam} href={food.recipe.shareAs} >View Edamam Nutritional Details</a>
          <p><span className={classes.bold}>Cuisine:</span> {food.recipe.cuisineType[0]}</p>
          <p><span className={classes.bold}>Meal Type:</span> {food.recipe.mealType[0]}</p>
          <p><span className={classes.bold}>Calories:</span> {Math.round(food.recipe.calories)}</p>
          <p><span className={classes.bold}>Ingredients List:</span> </p>
          {food.recipe.ingredientLines.map((ingredient) => {
            return <p className={classes['ingredients-list']} key={uuidv4()}>{ingredient}</p>;
          })}
        </CardContent>
      </Collapse>
    </Card>
